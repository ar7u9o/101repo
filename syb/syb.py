...
class Infix:
    ...
    def __call__(self, value1, value2):
        print 'call', self.function, value1, value2
...
def everywhereButTD(q, f, x):
    ...
    if q(x):
        return x
    return gmapT(everythingButTD(q, f),  f(x))
...
def gmapT(f, val):
    if type(val) in [int, str, float]:
        ...
        keys = filter(lambda k: not k.startswith('_'), keys)
...
def gmapQ(f, val):
    if type(val) in [int, str, float]:
        ...
        keys = dir(val)
        keys = filter(lambda k: not k.startswith('_'), list(set(keys) - set(dir(object))))
        items = [getattr(val, key) for key in keys]
